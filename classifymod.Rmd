...
```{r log, echo=TRUE, fig.height=6, fig.cap="Species presence/absence and mean annual temperature with linear regression", fig.alt="A plot with y-axis labelled 'Species occurance' and x-axis labelled 'Mean annual temperature (°C)' with a fitted straight line trending upwards"}
...
```{r logfit, echo=TRUE, fig.height=6, fig.cap="Species presence/absence and mean annual temperature with logistic regression", fig.alt="A plot with y-axis labelled 'Species occurance' and x-axis labelled 'Mean annual temperature (°C) with a red logistic regression line"}
...
```{r multilog, fig.cap="Species occurrence vs temperature and land classification as urban or rural", fig.alt="A plot with y-axis labelled 'Species occurrence' and x-axis labelled 'Mean annual temperature (°C). There are two logistic regression lines that represent the land categories."}
...
```{r, classlda, fig.cap="Classification of the iris dataset using LDA", fig.alt="Graph of two LDA axses with symbols for each species where the colour of the symbol indicates the classification produced by the LDA model.", echo=TRUE, comment=""}
...
```{r ldahist, fig.height=8,fig.cap="Histogram of the synthetic axis coordinates for each species", fig.alt="Three histograms for 'setosa', 'versicolor', and 'virginica' with bars concentrated from the right to left respectively.",  echo=TRUE,comment=""}

ldahist(plda$x[,1], g=plda$'class', col=2)
...
```{r firstree, echp=TRUE, message=FALSE,fig.width=10, fig.height=7., fig.cap="A simple plot of a tree classifier for the iris data", fig.alt="One classification tree with branches for options 'yes' and 'no' for Petal.Length<2.6 and Petal.width>1.75",echo=TRUE, comment=""}
library(rpart)
...
```{r fancytree, fig.cap="A nicer plot of a tree classifier for the iris data made with the rpart.plot package", fig.alt="A classification tree using the iris dataset. The top right of the figure shows a legend with labels 'setosa', 'versicolor' and 'virginica'. Contains branches for options 'yes' and 'no' for Petal.Length<2.6 and Petal.width>1.8",echo=TRUE, comment=""}
library(rpart.plot)
...
```{r rfoob, fig.cap="Out of bag error, and individual classification errors for the three species classes in the random forest model. We see large fluctuations in error at the beginning and middle which flatline as more trees are added", fig.alt="A plot with y-axis labelled 'Error' and x-axis labelled 'trees'. There are seperate lines labelled 'OOB', 'versicolor' and 'virginica' which show some fluctuations in error with fewer trees, while 'setosa' has no error and is parallel with the x-axis."}
plot(RF.model, main="",lwd=2, cex=1.2,cex.lab=1.3)
legend("topright", c("OOB", "setosa", "versicolor", "virginica"), col=c(1,2,3,4), lty=c(1,2,3,4), bty="n", lwd=2, cex=1.2)
...
```{r vipp, echo=TRUE, fig.cap="Variable importance plot for our random forest model of the iris data. The most important predictor is Petal.Width, followed by Petal.Length ", fig.alt="A plot with y-axis containing charcters of Petal.Length, Petal.Width, Septa.Length and Sepal.Width,  and x-axis labelled 'MeanDecreaseGini'"}
varImpPlot(RF.model, pch=16, cex=1.2) 
```
...
```{r PEplot, echo=TRUE, fig.cap="Partial effect of petal length in the random forest model of the iris data", fig.alt="A plot with y-axis labelled 'log odds' and x-axis labelled 'Petal.Length' with a downwards trending line."}
partialPlot(RF.model, iris, Petal.Length, main="", ylab="log odds", cex=1.2, cex.lab=1.3, lwd=2) 
```
...
```{r neuron, fig.height=4, fig.cap="Diagram showing the inputs, and internal components, including weighting, summation and activation function, that make up an aritifcial neuron", fig.alt="Inputs and weights are shown to sum together by a circle with a sigma symbol. The activation function is reperesented by a circle with an italicized 'f', conected by an arrow to the word output.", ECHO=FALSE}
...
```{r ann, fig.cap="Diagram of an artificial neural network. A box labelled 'input' contains two circles that represent different data, which are connected to three neurons represented as circles in the box labelled 'hidden layer'. The neurons point to the output", fig.alt="Artificial neural network showing connections between inputs to hidden layer to output", echo=FALSE}
...
```{r fig.height=3.5, fig.width=7, results = "asis", echo=TRUE, message=FALSE, comment="", fig.cap="A neural network selected by our algorithm to classify the iris data. Two data inputs of Petal.Width and Petal.Length are connected to three neurons, which together work to classify the data. The numbers on connecting arrows represent the weight attributed to that connection", fig.alt="A diagram with two data input circles, connected to three neurons circles, in turn connected to three circles labelled with the species names 'virginica', 'versicolor','setosa' which represent classification probabilities"}
...
#use to predict
comp <-neuralnet::compute(nniris, iristest[-5])
...
```{r e4, echo=TRUE, fig.cap="Variable importance plot from the final random forest model selected by caret for the Pima.tr dataset. The variable labelled 'glu', which stands for glucose, is most important ", fig.alt="Variable importance is indicated by the mean decrease in the Gini index. The y axis has names of all the variables from the Pima.tr dataset.", comment="", message=FALSE}
library(randomForest)
library(caret)
#cross-validation with 5 folds
train_control <- trainControl(method = "cv", number = 5) 
...
varImpPlot(rf_Pima$finalModel, main="final random forest model from caret", cex=1.2, pch=16)
```
